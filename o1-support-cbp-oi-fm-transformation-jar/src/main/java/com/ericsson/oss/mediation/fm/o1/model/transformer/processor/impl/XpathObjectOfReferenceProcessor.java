...
public class XpathObjectOfReferenceProcessor extends AbstractModelProcessor {
    ...
    private String extractTokenValues(final String moduleKeyValues, final String moduleKey) {
        ...
        for (final String objectKeyVal : moduleKeyValues.split(COMMA)) {
            ...
            if (StringUtils.isNotBlank(moduleKey) && objectKeyVal.split(EQUALS_SIGN).length > 1) {
                if (isFirstAttribute) {
    ...
    public String extractNameAndNameSpace(final String tokenValue) {
        if (tokenValue.contains(COLON) && (!tokenValue.contains(LEFT_SQUARE_BRACE) ||
                tokenValue.indexOf(COLON) < tokenValue.indexOf(LEFT_SQUARE_BRACE) && tokenValue.contains(RIGHT_SQAURE_BRACE))) {
            return tokenValue.substring(tokenValue.indexOf(COLON) + 1);
        }
        return tokenValue;
    }

    private boolean checkForPattern(final String tokenValue) {
        final boolean hasLeftBrace = tokenValue.contains(LEFT_SQUARE_BRACE);
        final boolean hasRightBrace = tokenValue.contains(RIGHT_SQAURE_BRACE);
